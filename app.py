...
from pdfextract import OptimizedDocumentExtractor
    ...
    with col1:
        ...
        if uploaded_files:
            ...
            with st.expander("⚙️ Processing Options"):
                ...
                with col1:
                    st.session_state.processing_options['extract_images'] = st.checkbox(
                        "Extract Images", 
                        value=st.session_state.processing_options['extract_images'],
                        help="Extract embedded images from documents"
                    )
                    st.session_state.processing_options['extract_tables'] = st.checkbox(
                        "Extract Tables", 
                        value=st.session_state.processing_options['extract_tables'],
                        help="Extract tables from documents"
                    )
                    st.session_state.processing_options['extract_metadata'] = st.checkbox(
                        "Extract Metadata", 
                        value=st.session_state.processing_options['extract_metadata'],
                        help="Extract document metadata"
                    )
                    st.session_state.processing_options['enable_caching'] = st.checkbox(
                        "Enable Caching", 
                        value=st.session_state.processing_options['enable_caching'],
                        help="Cache extraction results for faster processing"
                    )
                
                with col2:
                    st.session_state.processing_options['chunk_size'] = st.number_input(
                        "Chunk Size (pages)", 
                        min_value=10, 
                        max_value=200, 
                        value=st.session_state.processing_options['chunk_size'],
                        help="Number of pages to process at once (for large files)"
                    )
                    st.session_state.processing_options['max_workers'] = st.number_input(
                        "Max Workers", 
                        min_value=1, 
                        max_value=8, 
                        value=4 if st.session_state.processing_options['max_workers'] is None else st.session_state.processing_options['max_workers'],
                        help="Maximum number of parallel processing threads"
                    )
                    st.session_state.processing_options['memory_limit_mb'] = st.number_input(
                        "Memory Limit (MB)", 
                        min_value=256, 
                        max_value=4096, 
                        value=st.session_state.processing_options['memory_limit_mb'],
                        help="Memory usage limit for processing"
                    )
            
            # Process button
            if st.button("🚀 Start Processing", type="primary", use_container_width=True):
                process_documents(uploaded_files, st.session_state.processing_options)
            ...
            for i, file_path in enumerate(file_paths):
                ...
                try:
                    ...
                    if file_ext == '.pdf':
                        ...
                        result = extractor.extract_docx_optimized(file_path)
                    elif file_ext == '.pptx':
                        result = extractor.extract_pptx_optimized(file_path)
                    elif file_ext in ['.xlsx', '.xls']:
                        result = extractor.extract_excel_optimized(file_path)
                    else:
                        continue
...
def display_page_content(file_name, data, selected_page):
    ...
    with col2:
        ...
        page_tables = [table for table in data.get('tables', []) 
                      if table.get('page_number', selected_page) == selected_page]
...
def display_results(results):
    ...
    total_tables = sum(len(data.get('tables', [])) for data in results.values())
            ...
            with st.expander(f"📄 {file_name}"):
                ...
                with col1:
                    ...
                    if data['file_type'] == 'PDF':
                        ...
                        st.write(f"**Fonts Used:** {', '.join(data.get('fonts_used', [])[:5])}")
                        if len(data.get('fonts_used', [])) > 5:
                            st.write(f"(+ {len(data['fonts_used']) - 5} more fonts)")
                        ...
                        st.write(f"**Rows:** {data.get('total_rows', 0):,}")
                        st.write(f"**Columns:** {data.get('total_columns', 0):,}")
        ...
        if total_tables > 0:
            ...
            for data in results.values():
                for table in data.get('tables', []):
                    method_counts[table.get('method', 'optimized_detection')] += 1
            ...
            for file_name, data in results.items():
                ...
                if tables:
                    ...
                    for i, table in enumerate(tables, 1):
                        with st.expander(f"Table {i} ({table.get('method', 'optimized_detection')}) - Page {table.get('page_number', 'N/A')}"):
                            
                            # Show table metadata
                            col1, col2 = st.columns(2)
                            with col1:
                                st.write(f"**Method:** {table.get('method', 'optimized_detection')}")
                                st.write(f"**Page:** {table.get('page_number', 'N/A')}")
                                st.write(f"**Confidence:** {table.get('confidence', 'medium')}")
                            with col2:
                                if 'shape' in table:
                                ...
                                for idx, img in enumerate(images):
                                    with cols[idx % 3]:
                                        try:
                                            ...
                                            if os.path.exists(img.get('filename', '')):
                                                st.image(
                                                    img['filename'],
                                                ...
                                                # Show placeholder if image file not found
                                                st.info(f"📸 {os.path.basename(img.get('filename', 'Image'))}")
                                            
                                            # Image metadata
                                            with st.expander(f"Image {idx + 1} Details"):
                                                st.write(f"**Filename:** {os.path.basename(img.get('filename', 'N/A'))}")
                ...
                with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
                    ...
                    for file_name, data in results.items():
                        ...
                        for i, table in enumerate(data.get('tables', []), 1):
                            try:
                                if table.get('data'):
                                    page_info = f"_page_{table.get('page_number', 'unknown')}" if table.get('page_number') else ""
                                    csv_name = f"{file_base}_table_{i}{page_info}_{table.get('method', 'optimized_detection')}.csv"
        ...
        if st.button("📊 Download Summary Report", use_container_width=True):
            ...
            for file_name, data in results.items():
                ...
                if data['file_type'] == 'PDF':
                    ...
                    summary_lines.append(f"    - Fonts: {', '.join(data.get('fonts_used', [])}")
                    ...
                    summary_lines.append(f"    - Rows: {data.get('total_rows', 0):,}")
                    summary_lines.append(f"    - Columns: {data.get('total_columns', 0):,}")
